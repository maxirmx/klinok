...
<template>
  <div class="instrumental-study-create">
    ...
    <div class="instrumental-study-create-control medical-card-action-grid">
      <AppCatalogCombobox v-model:selected-ids="pendingTypeIds" label="Тип исследования" :options="INSTRUMENTAL_STUDY_OPTIONS" custom-text="" :allow-custom="false" :invalid="Boolean(errors.section)" />
  ...
  <div class="instrumental-study-list">
    <section v-for="(study, index) in model.studies" :key="study.id" class="instrumental-study-card">
      ...
      <label class="instrumental-study-date"><span>Дата исследования</span><input v-model="study.date" type="date" :max="new Date().toISOString().slice(0, 10)" required :aria-invalid="invalid(errors.studies[index]?.date)" /><small v-if="errors.studies[index]?.date" class="field-error" role="alert">{{ errors.studies[index]?.date }}</small></label>
