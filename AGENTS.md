...
- Render epicrisis collections table-first on wide screens, matching the bordered header-and-row treatment used by pet doctor-access tables. Transform those same rows into bordered, field-labelled cards on narrow screens; do not prefer cards while the table fits. Reuse the shared epicrisis presentation in owner and doctor pet views, immediately after the pet profile and before other medical sections.
