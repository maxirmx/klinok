...
- Align every top-level block in owner and doctor pet-detail views to the same inline-start and inline-end edges. Do not inset or narrow an individual top-level panel relative to its siblings; keep any additional spacing inside the panel.
- In tables and table-like collection row layouts, align cell contents to the top on both wide and narrow screens. Mixed-height content such as status badges, identities, and action buttons must share the same top alignment rather than being vertically centered. Do not treat label-and-control form rows as table-like rows: on wide layouts, vertically center the visible label against its paired input, select, or textarea, including therapeutic fields, `.therapeutic-short-text`, and `.medical-card-comment-section`. Checkbox and radio labels, action rails, and actual table-like collections retain their explicit top-alignment rules.
- Render read-only tabular collections table-first on wide screens using the bordered `owner-access-table-wrap` and `owner-access-table` treatment established by pet doctor-access tables. Transform those same rows into bordered, field-labelled cards on narrow screens by supplying `data-label` for every cell; do not maintain a competing mobile-only data presentation or prefer cards while the table fits. Apply this pattern to epicrisis and laboratory-comparison tables. Reuse the shared epicrisis presentation in owner and doctor pet views, immediately after the pet profile and before other medical sections.
