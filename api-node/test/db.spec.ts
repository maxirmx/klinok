...
describe("PostgreSQL migrations", () => {
  it("defines ownership transfers with one pending request per pet and indexed party queues", async () => {
    ...
    expect(sql).toContain("pet_ownership_transfers_from_owner_history_idx");
    expect(sql).toContain("ON pet_ownership_transfers(from_owner_account_id, created_at DESC)");
    expect(sql).toContain("pet_ownership_transfers_to_owner_history_idx");
    expect(sql).toContain("ON pet_ownership_transfers(to_owner_account_id, created_at DESC)");
    expect(sql).not.toContain("pet_ownership_transfers_parties_idx");
  });
