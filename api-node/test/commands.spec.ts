...
import { CommandService, validateMedicalEncounter } from "../src/commands.js";
...
describe("command boundary", () => {
  ...
  it("serializes concurrent transfer acceptance and grant revocation on the pet lock", async () => {
    const pet = {
      pet_id: "pet-1", owner_account_id: "owner-1", revision: 4, name: "Ёжик", species: "Кошка",
      breed: "Домашняя", created_at: timestamp, updated_at: timestamp, deleted_at: null,
    } as Record<string, unknown>;
    const transfer = pendingTransfer("owner-1");
    const grant = {
      grant_id: "grant-1", pet_id: "pet-1", grantor_account_id: "owner-1", grantee_account_id: "doctor-1",
      grantee_display_name: "Иван Врач", actions: ["read"], request_id: null, parent_grant_id: null,
      revision: 1, status: "active", created_at: timestamp,
    } as Record<string, unknown>;
    const profiles: Record<string, Record<string, unknown>> = {
      "owner-1": { account_id: "owner-1", email: "one@example.ru", credential_status: "active", profile_revision: 2, first_name: "Алёна", last_name: "Ёлкина", patronymic: null, updated_at: timestamp },
      "owner-2": { account_id: "owner-2", email: "two@example.ru", credential_status: "active", profile_revision: 3, first_name: "Иван", last_name: "Петров", patronymic: null, updated_at: timestamp },
    };
    let resolveTransferPetLocked!: () => void;
    let resolveAccessPetWaiting!: () => void;
    let allowTransferToContinue!: () => void;
    const transferPetLocked = new Promise<void>((resolve) => { resolveTransferPetLocked = resolve; });
    const accessPetWaiting = new Promise<void>((resolve) => { resolveAccessPetWaiting = resolve; });
    const transferMayContinue = new Promise<void>((resolve) => { allowTransferToContinue = resolve; });
    const lockOwners = new Map<string, number>();
    const lockWaiters = new Map<string, { transactionId: number; resolve: () => void }[]>();
    let transactionCount = 0;

    async function acquireLock(transactionId: number, key: string): Promise<void> {
      const owner = lockOwners.get(key);
      if (owner === undefined) {
        lockOwners.set(key, transactionId);
        return;
      }
      if (owner === transactionId) return;
      if (transactionId === 2 && key === "pet:pet-1") resolveAccessPetWaiting();
      await new Promise<void>((resolve) => {
        const waiters = lockWaiters.get(key) ?? [];
        waiters.push({ transactionId, resolve });
        lockWaiters.set(key, waiters);
      });
    }

    function releaseLocks(transactionId: number): void {
      for (const [key, owner] of lockOwners) {
        if (owner !== transactionId) continue;
        const waiters = lockWaiters.get(key) ?? [];
        const next = waiters.shift();
        if (next) {
          lockOwners.set(key, next.transactionId);
          next.resolve();
          if (waiters.length) lockWaiters.set(key, waiters);
          else lockWaiters.delete(key);
        } else {
    ...
    const database = { transaction: vi.fn(async (work: (client: {
      ...
      const query = vi.fn(async (sql: string, params: unknown[] = []) => {
        ...
        if (sql.startsWith("SELECT credential_status")) return { rows: [{ credential_status: "active" }], rowCount: 1 };
        if (sql.startsWith("SELECT 1 FROM roles")) return { rows: [{}], rowCount: 1 };
        if (sql.startsWith("SELECT pet_id,from_owner_account_id")) return { rows: [transfer], rowCount: 1 };
        if (sql.startsWith("SELECT pet_id FROM access_grants")) return { rows: [{ pet_id: grant.pet_id }], rowCount: 1 };
        if (sql.startsWith("SELECT * FROM pets WHERE pet_id")) {
          await acquireLock(transactionId, "pet:pet-1");
          if (transactionId === 1) {
            resolveTransferPetLocked();
            await transferMayContinue;
          }
          return { rows: [pet], rowCount: 1 };
        }
        if (sql.startsWith("SELECT a.account_id")) {
          const ids = params[0] as string[];
          return { rows: ids.map((id) => profiles[id]!), rowCount: ids.length };
        }
        if (sql.startsWith("SELECT * FROM pet_ownership_transfers WHERE transfer_request_id")) {
          await acquireLock(transactionId, "transfer:transfer-1");
          return { rows: [transfer], rowCount: 1 };
        }
        if (sql.startsWith("SELECT * FROM access_grants WHERE grant_id")) {
          await acquireLock(transactionId, "grant:grant-1");
          return { rows: [grant], rowCount: 1 };
        }
        if (sql.startsWith("SELECT t.*,p.name AS pet_name")) return { rows: [{
          ...transfer, pet_name: pet.name, pet_species: pet.species,
          from_owner_display_name: "Алёна Ёлкина", to_owner_display_name: "Иван Петров",
        }], rowCount: 1 };
        if (sql.startsWith("SELECT DISTINCT a.email")) return { rows: [], rowCount: 0 };
        if (sql.startsWith("UPDATE pets SET owner_account_id")) {
          pet.owner_account_id = params[1];
          pet.revision = Number(pet.revision) + 1;
          return { rows: [pet], rowCount: 1 };
        }
        if (sql.startsWith("UPDATE access_requests")) return { rows: [], rowCount: 0 };
        if (sql.startsWith("UPDATE access_grants SET status='revoked'")) {
          await acquireLock(transactionId, "grant:grant-1");
          grant.status = "revoked";
          grant.revision = Number(grant.revision) + 1;
          return { rows: [], rowCount: 1 };
        }
        if (sql.startsWith("UPDATE pet_ownership_transfers SET status='completed'")) {
          transfer.status = "completed";
          transfer.revision = Number(transfer.revision) + 1;
          transfer.decided_by = params[1];
          return { rows: [], rowCount: 1 };
        }
        if (sql.startsWith("SELECT email FROM accounts")) return { rows: [], rowCount: 0 };
        if (sql.startsWith("INSERT INTO email_outbox") || sql.startsWith("INSERT INTO operation_receipts")) {
          return { rows: [], rowCount: 1 };
        }
        throw new Error(`Unexpected SQL: ${sql}`);
      });
    ...
    const acceptance = service.execute({ accountId: "owner-2" }, {
      operationId: "concurrent-transfer", type: "transfer.accept", activeRole: "owner", entityId: "transfer-1",
      expectedRevision: 1, createdAt: timestamp, payload: {},
    });
    await transferPetLocked;
    const revocation = service.execute({ accountId: "owner-1" }, {
      operationId: "concurrent-revocation", type: "access.revoke", activeRole: "owner", entityId: "grant-1",
      expectedRevision: 1, createdAt: timestamp, payload: {},
    });
    await accessPetWaiting;
    allowTransferToContinue();

    await expect(Promise.all([acceptance, revocation])).resolves.toEqual([
      expect.objectContaining({ status: "applied" }),
      expect.objectContaining({ status: "rejected", error: expect.objectContaining({ code: "GRANT_NOT_ACTIVE" }) }),
    ]);
  });

  it.each([
    ["missing", undefined, "GRANT_NOT_ACTIVE", "rejected"],
    ["retargeted", { pet_id: "pet-2", status: "active", revision: 1 }, "REVISION_CONFLICT", "conflict"],
  ] as const)("rejects a %s grant after its pet is resolved", async (_case, lockedGrant, code, status) => {
    const query = vi.fn(async (sql: string) => {
      if (sql.startsWith("SELECT pg_advisory_xact_lock")) return { rows: [], rowCount: 1 };
      if (sql.startsWith("SELECT actor_account_id")) return { rows: [], rowCount: 0 };
      if (sql.startsWith("SELECT credential_status")) return { rows: [{ credential_status: "active" }], rowCount: 1 };
      if (sql.startsWith("SELECT pet_id FROM access_grants")) {
        return lockedGrant ? { rows: [{ pet_id: "pet-1" }], rowCount: 1 } : { rows: [], rowCount: 0 };
      }
      if (sql.startsWith("SELECT * FROM pets")) return { rows: [{ pet_id: "pet-1", deleted_at: null }], rowCount: 1 };
      if (sql.startsWith("SELECT * FROM access_grants")) return { rows: [lockedGrant], rowCount: 1 };
      throw new Error(`Unexpected SQL: ${sql}`);
    });
    const client = { query };
    const database = { transaction: vi.fn(async (work: (value: typeof client) => Promise<unknown>) => work(client)) };
    const ledger = { isValid: vi.fn(() => true), append: vi.fn(), noteCommitted: vi.fn() } as unknown as Ledger;
    const service = new CommandService(database as never, ledger);

    await expect(service.execute({ accountId: "owner-1" }, {
      operationId: `grant-${_case}`, type: "access.revoke", activeRole: "owner", entityId: "grant-1",
      expectedRevision: 1, createdAt: timestamp, payload: {},
