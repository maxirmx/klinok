...
"${compose[@]}" up -d postgres mail adminer
...
printf '\nKlinok v3 backend is ready.\nAdminer: http://localhost:%s\nStop it with: COMPOSE_PROJECT_NAME=%s docker compose down\nStarting Vite at http://localhost:8080\n' "$KLINOK_ADMINER_PORT" "$COMPOSE_PROJECT_NAME"
npm run dev
