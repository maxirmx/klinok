...
describe("PetColorCombobox", () => {
  it("toggles and dismisses the complete dog color list accessibly", async () => {
    ...
    const toggle = field.get(".owner-color-toggle");
    ...
    expect(field.findAll(".owner-color-option").map((option) => option.text())).toEqual(dogColors);

    await toggle.trigger("click");
    expect(field.find(".owner-color-options").exists()).toBe(false);
    expect(toggle.getComponent(AppIcon).props("name")).toBe("chevron-down");
  ...
  it("filters cat colors case-insensitively and supports mouse and keyboard selection", async () => {
    ...
    await toggle.trigger("click");
    expect(field.findAll(".owner-color-option").map((option) => option.text())).toEqual(catColors);
    await toggle.trigger("click");

    await input.setValue("ПОЙНТ");
    expect(field.findAll(".owner-color-option").map((option) => option.text())).toEqual([
      "Колор-пойнт",
      "Сил-пойнт",
      "Блю-пойнт",
      "Ред-пойнт",
    ...
    expect(field.get(".owner-color-empty").text()).toBe("Нет подходящих вариантов");
    expect(field.findAll(".owner-color-option")).toHaveLength(0);

    await input.setValue("");
    await field.findAll(".owner-color-option").find((option) => option.text() === "Ситцевый")!.trigger("click");
